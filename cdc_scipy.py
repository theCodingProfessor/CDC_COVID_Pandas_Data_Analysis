# Clinton Garwood
# Analysis of CDC Data using SciPy
# Spring 2022; CSCE 364
...
# print version of scipy used
print("scipy version is ", si.__version__)
...
def import_data():
    ...
    # Explicitly create a pandas data frame
    cdc_df = pd.DataFrame(df_cdc_all)
    # Explicitly create a numpy array
    # why doesn't this work? np can only handle numeric data (here state names are included)
    # cdc_np = np.array(df_cdc_all)
    return cdc_df
...
def getmaxdate_np(df_cdc_using):
    df_cdc_for_np = df_cdc_using.loc[:,["submission_date"]]
    cdc_np_raw = np.array([df_cdc_for_np])
    cdc_np = np.array([cdc_np_raw.dtype('M')])
    print(cdc_np)
    print(cdc_np.ndim)
    print("The dimensions in np are:", cdc_np.ndim)
    max_date_np = np.max(a=cdc_np,axis=0)
    print("Max Date from NP is: ", max_date_np)
    return max_date_np
...
def main():
    ...
    max_date_np = getmaxdate_np(df_cdc_using)
